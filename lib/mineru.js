...
function pruneMineruCache(dataDir, keepCacheId) {
  ...
  for (const item of directories) {
    const mustKeep = item.id === keepCacheId;
    if (mustKeep || (kept < MAX_CACHE_ENTRIES && total + item.size <= MAX_CACHE_BYTES)) {
      kept += 1;
      total += item.size;
      continue;
    }
...
function writeMineruCache(entries, dataDir, cacheId, referencedPaths) {
  const cacheRoot = path.join(dataDir, "mineru-cache", cacheId);
  const wanted = new Set([...referencedPaths].map((item) => String(item).toLowerCase()));
  if (!wanted.size) {
    pruneMineruCache(dataDir, cacheId);
    return cacheRoot;
  }
...
async function parseMineruAttempt({ buffer, normalizedFileName, ctx, options, isOcr, attemptIndex }) {
  ...
  while (Date.now() < timeoutAt) {
    ...
    if (current.state === "failed") {
      throw new MineruStageError("MinerU 任务解析", text(current.err_msg, "MinerU 解析失败"), true);
    }
  }
  ...
  await runMineruStage("缓存 MinerU 视觉资源", () => writeMineruCache(entries, ctx.dataDir, cacheId, referencedPaths));
...
export function readMineruAsset({ ctx, cacheId, assetPath }) {
  ...
  const root = path.resolve(ctx.dataDir, "mineru-cache", cacheId);
