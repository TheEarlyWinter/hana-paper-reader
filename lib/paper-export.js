...
function renderBlock(block, index, translations, assets, options) {
  ...
  const lines = [`<a id="${anchor}"></a>`, `<!-- page:${page} block:${escapeMarkdown(id || String(index + 1))} -->`, ""];

  if (block.type === "heading" && original) {
    lines.push(`${"#".repeat(headingLevel(block))} ${escapeInlineMarkdown(original)}`);
    if (translated && translated !== original) lines.push("", `> **译文：** ${escapeMarkdown(translated)}`);
    return lines;
  }
  ...
  if (VISUAL_TYPES.has(block.type)) {
    ...
    if (translated) lines.push("", `**译文：** ${escapeMarkdown(translated)}`);
    return lines;
  }
...
export function generatePaperMarkdown(input = {}) {
  ...
  if (blocks.length) {
    const body = ["## 正文", ""];
    blocks.forEach((block, index) => {
      body.push(...renderBlock(block, index, translations, assets, options), "");
    });
