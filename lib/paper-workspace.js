...
} from "./paper-evidence.js?hpr=0.8.0-r1";
import { createPaperStorage, STORAGE_LAYOUT } from "./paper-storage.js?hpr=0.8.0-r1";
...
// A plugin can have more than one route/workspace instance after a WebView
// reload. Keep mutations for the same on-disk workspace in one process-wide
// queue so a stale instance cannot overwrite a newer glossary or note.
...
export function createPaperWorkspace(options = {}) {
  ...
  const api = {
    ...
    async putTranslation(input = {}) { const hash = safePaperHash(input.paperHash); const blockId = safeBlockId(input.blockId); const glossaryVersion = Number.isInteger(input.glossaryVersion) ? input.glossaryVersion : 0; const key = translationCacheKey(hash, blockId, glossaryVersion, input); return mutate((store) => { store.translationCache[key] = timestampPatch(object(input), { key, paperHash: hash, blockId, glossaryVersion, agentId: text(input.agentId, 128) || null, modelRef: text(input.modelRef, 512) || null, source: text(input.source), translation: text(input.translation), createdAt: store.translationCache[key]?.createdAt || now() }); return store.translationCache[key]; }); },
