...
assert.match(panelSource, /UI_VERSION = "0\.6\.1"/);
for (const mode of ["original", "bilingual", "translation"]) assert.match(panelSource, new RegExp(`data-reading-mode=\\"${mode}\\"`));
...
assert.match(apiSource, /PLUGIN_API_VERSION = "0\.6\.1"/);
assert.match(apiSource, /paper-export\.js\?hpr=0\.6\.1/);
assert.match(apiSource, /paper-workspace\.js\?hpr=0\.6\.1/);
...
assert.match(read("routes/ui.js"), /ASSET_VERSION = "0\.6\.1-r1"/);
...
assert.equal(publicSettings.apiVersion, "0.6.1");
