...
assert.match(panelSource, /UI_VERSION = "0\.5\.0"/);
...
assert.match(apiSource, /PLUGIN_API_VERSION = "0\.5\.0"/);
assert.match(apiSource, /paper-export\.js\?hpr=0\.5\.0-final/);
assert.match(apiSource, /paper-workspace\.js\?hpr=0\.5\.0-final/);
...
assert.match(cssSource, /\.reader-container\s*\{\s*flex-direction:\s*column;/s);
...
assert.equal(publicSettings.apiVersion, "0.5.0");
